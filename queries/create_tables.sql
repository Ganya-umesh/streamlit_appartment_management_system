...
--TRIGGER AGAIN FOR RESIDENT
DELIMITER //

CREATE TRIGGER add_resident_auth AFTER INSERT ON resident
FOR EACH ROW
BEGIN
    INSERT INTO auth (username, password, type)
    VALUES (NEW.Aadhar, NEW.Aadhar, 'res');
END;
//

DELIMITER ;

--TRIGGER FOR SECURITY
DELIMITER //

CREATE TRIGGER add_security_auth AFTER INSERT ON security
FOR EACH ROW
BEGIN
    INSERT INTO auth (username, password, type)
    VALUES (NEW.SecurityID, NEW.SecurityID, 'sec');
END;
//
