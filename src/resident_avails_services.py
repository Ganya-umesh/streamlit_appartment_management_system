...
def update_ras():
    ...
    list_ras = [(f"{row['ResidentUID']}-{row['serviceID']}", row['serviceTime']) for index, row in df.iterrows()]

    selected_ras = st.selectbox("Select a resident availed service to edit:", list_ras, format_func=lambda x: x[0])
    if selected_ras:
        ...
        st.warning("Please select a resident availed service to edit.")
