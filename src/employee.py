...
from database import add_employee,view_all_serviceids,edit_employee,view_all_employees,drop_employee,get_employee,view_all_employees_resident
...
def show_employees():
    ...
    df=pd.DataFrame(result,columns=["employeeID","Fname","Lanme","phone","shift","serviceID","type","cost", "salary"],dtype=str)
    st.table(df)


def show_employees_resident():
    result=view_all_employees_resident()
    emp_og_list = ["employeeID","Fname","Lname","phone","doj","gender","shift","serviceID","salary"]
    df=pd.DataFrame(result,columns=["employeeID","Fname","Lname","phone","shift","serviceID","type","cost"],dtype=str)
    st.table(df)
    ...
    if selected_result:
        ...
        with st.expander("Edit employee:"):
            ...
            if st.button("Update employee"):
                edit_employee(new_eid, new_fname, new_lname, new_phone,str(new_doj), new_gender, new_shift, new_serviceid, new_salary, eid)
