...
#Procedue#########################################################################################
# Procedure execution function
def get_residents_with_dependents_count():
    c.callproc("get_residents_with_dependents_count")
    # Fetch all results from the result set
    result = []
    for result_set in c.stored_results():
...
def view_all_employees():
    c.execute("""
            SELECT emp.employeeID,emp.Fname,emp.Lname,emp.phone,emp.shift,emp.serviceID, svc.type, svc.cost,emp.salary
            FROM employee emp
...
def view_all_employees_resident():
    c.execute("""
            SELECT emp.employeeID,emp.Fname,emp.Lname, emp.phone,emp.shift,emp.serviceID, svc.type, svc.cost
            FROM employee emp
            JOIN services svc ON emp.serviceID = svc.serviceID;"""
        )
    data=c.fetchall()
